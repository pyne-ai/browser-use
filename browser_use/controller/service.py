...
class Controller:
    ...
    def _register_default_actions(self):
        ...
        @self.registry.action(
            "Switch tab", param_model=SwitchTabAction, requires_browser=True
        )
                    ...
                    try:
                        ...
                        if dropdown_info:
                            ...
                            if result.get("success"):
                                msg = f"Selected option '{text}' (value={result.get('selectedValue')}"
                                ...
                                if "availableOptions" in result:
                                    logger.error(
                                        f"Available options: {result['availableOptions']}"
                                    )
    ...
    #     return ActionResult(
    #         extracted_content=f"Checkpoint saved {ckpt.url}", include_in_memory=True
    #     )
    ...
    async def act(
        self, action: ActionModel, browser_context: BrowserContext
    ) -> ActionResult:
        """Execute an action"""
        try:
